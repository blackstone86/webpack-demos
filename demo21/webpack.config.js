...
module.exports = {
  ...
  plugins: [
    ...
    new OfflinePlugin({
      ...
      caches: {
        // 配置缓存模块，变更缓存模块的资源文件列表，之前的旧版本缓存会被自动删除，新版本缓存启用。缓存更新不需要重启 webpack-dev-server
        main: [
          // serviceworker install 事件中缓存资源，最早被缓存，缓存优先级最高，缓存应用最重要的资源（缺少会导致应用运行不了），一旦缓存失败会导致应用不缓存任何资源
          'main_*.js',
          'main_*.css',
          'index.html'
        ],
        additional: [
          // serviceworker activate 事件中缓存资源，缓存时间晚于 main 缓存模块，优先级次于 main 缓存模块，一旦缓存失败会将缓存失败的资源迁移到 optional 模块
          // '*.woff',
          // '*.woff2'
        ],
        optional: [
          // 当资源向服务器发起请求时缓存，:rest: 关键字匹配 main、additional 缓存模块以外的所有静态资源
          ':rest:'
        ]
      },
      ServiceWorker: {
